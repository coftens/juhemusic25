...
public class AudioPlayer implements MethodCallHandler, Player.Listener, MetadataOutput {
    ...
    private MediaSource decodeAudioSource(final Object json) {
        ...
        case "progressive":
            return new ProgressiveMediaSource.Factory(buildDataSourceFactory(mapGet(map, "headers"), (String)map.get("uri")), buildExtractorsFactory(mapGet(map, "options")))
                    .createMediaSource(new MediaItem.Builder()
                            .setUri(Uri.parse((String)map.get("uri")))
                            .setTag(id)
                            .build());
        case "dash":
            return new DashMediaSource.Factory(buildDataSourceFactory(mapGet(map, "headers"), (String)map.get("uri")))
                    .createMediaSource(new MediaItem.Builder()
                            .setUri(Uri.parse((String)map.get("uri")))
                            ...
                            .build());
        case "hls":
            return new HlsMediaSource.Factory(buildDataSourceFactory(mapGet(map, "headers"), (String)map.get("uri")))
                    .createMediaSource(new MediaItem.Builder()
                            .setUri(Uri.parse((String)map.get("uri")))
    ...
    private DataSource.Factory buildDataSourceFactory(Map<?, ?> headers, String uri) {
        ...
        Log.e(TAG, "Using UserAgent: " + userAgent);
        ...
        if (uri != null) {
             ...
             if (lower.contains("126.net") || lower.contains("163.com")) {
                 nativeHeaders.put("Referer", "https://music.163.com/");
                 Log.e(TAG, "Injecting NetEase Referer");
             } else if (lower.contains(".qq.com")) {
                 nativeHeaders.put("Referer", "https://y.qq.com/");
                 Log.e(TAG, "Injecting QQ Music Referer");
             }
        }
        
        if (stringHeaders != null && stringHeaders.size() > 0) {
            nativeHeaders.putAll(stringHeaders);
        }
        if (!nativeHeaders.isEmpty()) {
            httpDataSourceFactory.setDefaultRequestProperties(nativeHeaders);
        }
