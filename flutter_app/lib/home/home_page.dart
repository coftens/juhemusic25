...
class _HomePageState extends State<HomePage> with SingleTickerProviderStateMixin {
  ...
  List<_HomeSong> _dailySongs = const [];
...
class _RecPageContent extends StatelessWidget {
  ...
  final List<_HomeSong> dailySongs;
  final Future<void> Function() refresh;

  const _RecPageContent({required this.playlists, required this.songs, required this.dailySongs, required this.refresh});
...
class _HeroCards extends StatelessWidget {
  const _HeroCards({required this.playlists, required this.songs, required this.dailySongs});
        ...
        children: [
          Expanded(
            child: _HeroCard(
              title: '每日推荐',
              subtitle: '今日限定好歌推荐',
              // Use the first song's cover from the WYY daily recommend list
              coverUrl: dailySongs.isNotEmpty ? dailySongs[0].coverUrl : (playlists.isNotEmpty ? playlists[0].coverUrl : ''),
              dark: true,
              onTap: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (_) => PlaylistPage(
                      ...
                      coverUrl: dailySongs.isNotEmpty ? dailySongs[0].coverUrl : '',
                    ),
                  ),
                );
              },
