...
class PlayerService extends ChangeNotifier {
  ...
  // Expose player for TogetherService
  ...
  List<SearchItem> get effectiveQueue {
    if (_queue.isEmpty) return const [];
    if (_order.isEmpty || _order.length != _queue.length) return _queue;
    return [for (final i in _order) _queue[i]];
  }

  Future<void> jumpToOrderPos(int pos) async {
    if (_queue.isEmpty || _order.isEmpty) return;
    if (pos < 0 || pos >= _order.length) return;
    final originalIndex = _order[pos];
    await jumpTo(originalIndex);
  }

  void _rebuildOrder({required int startIndex}) {
    print('[PlayerService] _rebuildOrder: playMode=$_playMode, startIndex=$startIndex, queueLen=${_queue.length}');
    if (_queue.isEmpty) {
      _order = const [];
      _orderPos = 0;
      return;
    ...
    if (_playMode == 'shuffle') {
      // Smart Shuffle: Prioritize unplayed songs (index > startIndex)
      final unplayed = <int>[];
      final played = <int>[];
      ...
      // Multi-pass De-Clumping on the FINAL _order list
      // We run multiple passes to ensure no sequential pairs remain.
      final random = Random();
      for (var pass = 0; pass < 5; pass++) {
        bool changed = false;
        for (var i = 0; i < _order.length - 1; i++) {
          if ((_order[i+1] - _order[i]).abs() == 1) {
            // Found sequential pair! Find a good swap target.
            // Search for a position j (j != i, j != i+1) such that swapping _order[i+1] with _order[j]
            // doesn't create new conflicts at position i or j-1.
            for (var attempt = 0; attempt < 20; attempt++) {
              final j = random.nextInt(_order.length);
              if (j == i || j == i + 1) continue;
              
              // Check: would _order[j] next to _order[i] be OK?
              if ((_order[j] - _order[i]).abs() == 1) continue;
              
              // Check: would _order[i+1] next to _order[j-1] be OK? (if j > 0)
              if (j > 0 && (_order[i+1] - _order[j-1]).abs() == 1) continue;
              
              // Check: would _order[i+1] next to _order[j+1] be OK? (if j < length-1)
              if (j < _order.length - 1 && (_order[i+1] - _order[j+1]).abs() == 1) continue;
              
              // Swap!
              final temp = _order[i+1];
              _order[i+1] = _order[j];
              _order[j] = temp;
              changed = true;
              break;
        ...
        if (!changed) break; // No more conflicts found
      }
      
      print('[PlayerService] _rebuildOrder(SmartShuffle): New Order=${_order.take(10).toList()}..., OrderPos=0');
    } else {
      _order = [for (var i = 0; i < n; i++) i];
      _orderPos = startIndex;
      print('[PlayerService] _rebuildOrder(Seq/Repeat): OrderPos=$startIndex');
    }
  }
    ...
    if (idx > expectedIndex) {
      ...
      if (hasNext) {
        ...
        if (_playMode == 'repeat_one') {
          // If repeat one, we don't advance the queue index, we just replay current.
          // But wait, if system moved index forward, we might need to seek back or something?
          // Actually, if we just playItem(_queue[_index]), it should reset.
          // BUT, we shouldn't call next() which increments orderPos.
          debugPrint('System Next -> Repeat One active, replaying current');
          playItem(_queue[_index]); 
        } else {
           next();
        }
      }
  ...
  bool disableAutoAdvance = false; // Together Mode Hook

  Future<void> _handleTrackCompleted() async {
    if (_player == null) return;
    
    // Together Mode Hook: Guest should not auto-advance
    if (disableAutoAdvance) {
      print('[PlayerService] Auto-advance disabled (Together Mode Guest). Pausing at end of track.');
      await _player?.pause();
      // Do NOT seek to zero, just stop at end.
      return;
    }

    if (_queue.isEmpty) return;

    print('[PlayerService] _handleTrackCompleted: Mode=$_playMode, hasNext=$hasNext, OrderPos=$_orderPos/${_order.length}');

    try {
      if (_playMode == 'repeat_one') {
        print('[PlayerService] _handleTrackCompleted: Looping current song');
  ...
  void setPlayMode(String mode) {
    ...
    print('[PlayerService] setPlayMode: $mode (current=$_playMode)');
    ...
    // Refresh player to ensure "Next" item in ConcatenatingAudioSource is correct
    // preventing the "Blip" of the old sequential song.
    if (_player != null && _current != null) {
       final pos = _player!.position;
       playItem(_current!, startAt: pos);
    }
  }
  ...
  String _getEffectiveQuality(String platform, String pref) {
    ...
    if (platform == 'wyy') {
      // WYY优先使用用户手动选择的音质 (pref argument)
      if (_qualities.containsKey(pref)) {
        return pref;
      }
      // 回退：按优先级顺序选择
  ...
  Future<void> playItem(SearchItem item, {bool autoPlay = true, String? quality, Duration? startAt}) async {
    _ensurePlayer();
    ...
    try {
      ...
      // Use _current (updated with coverUrl) if available, otherwise fallback to item
      final itemWithCover = _current ?? item;
      
      final tag = MediaItem(
        id: itemWithCover.shareUrl,
        title: itemWithCover.name,
        artist: itemWithCover.artist,
        artUri: itemWithCover.coverUrl.isNotEmpty ? Uri.tryParse(itemWithCover.coverUrl) : null,
        extras: {'platform': itemWithCover.platform},
      );
      ...
      // 1. Add Prev (if exists AND not disabled)
      if (hasPrev && !disableAutoAdvance) {
         final prevIndex = _order[_orderPos - 1];
         final prevItem = _queue[prevIndex];
      ...
      // 3. Add Next (if exists AND not disabled)
      if (hasNext && !disableAutoAdvance) {
        // Use _order to find the actual next song, not just _index + 1
        final nextIndex = _order[_orderPos + 1];
        final nextItem = _queue[nextIndex];
        sources.add(AudioSource.uri(
          Uri.parse(picked), // Reuse current URI (placeholder, just_audio prefetches)
          tag: MediaItem(
  ...
  Future<void> next() async {
    print('[PlayerService] next() called. hasNext=$hasNext');
    if (hasNext) {
      _orderPos += 1;
      _index = _order[_orderPos];
      print('[PlayerService] next() -> Playing index $_index (OrderPos $_orderPos)');
      await playItem(_queue[_index]);
      return;
    }
    print('[PlayerService] next() -> No next song, trying recommendations');
  ...
  Future<void> _refillFromSourceList() async {
    ...
    // 加载下10首
    final nextItems = ctx.originalList.skip(ctx.loadedCount).take(10).toList();
