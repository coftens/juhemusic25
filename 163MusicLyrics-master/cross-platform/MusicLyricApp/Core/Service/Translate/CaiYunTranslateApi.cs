...
public class CaiYunTranslateApi : TranslateCacheableApi
{
    ...
    public CaiYunTranslateApi(string token)
    {
        ...
        {
            throw new MusicLyricException(ErrorMsgConst.CAIYUN_TRANSLATE_AUTH_FAILED);
        }
        ...
        LanguageEnum outputLanguage)
    {
        ...
        foreach (var input in ChunkArray(inputs, HTTP_CHUNK_SIZE))
        {
            ...
            var result = JsonConvert.DeserializeObject<CaiYunTranslateResult>(resultStr);

            if (result.Target == null)
            {
                return ResultVo<string[]>.Failure(ErrorMsgConst.CAIYUN_TRANSLATE_AUTH_FAILED);
            }
