...
public class BaiduTranslateApi : TranslateCacheableApi
{
    ...
    public BaiduTranslateApi(string appId, string secret, int delay = 1000)
    {
        ...
        {
            throw new MusicLyricException(ErrorMsgConst.BAIDU_TRANSLATE_AUTH_FAILED);
        }
    ...
    protected override ResultVo<string[]> Translate0(string[] inputs, LanguageEnum inputLanguage,
        LanguageEnum outputLanguage)
    {
        ...
        foreach (var subInputs in inputsList)
        {
            ...
            var md5Byte = md5.ComputeHash(Encoding.UTF8.GetBytes(_appId + source + _last.Millisecond + _secret)) ??
                          throw new ArgumentNullException(
                              "md5.ComputeHash(Encoding.UTF8.GetBytes(_appId + source + last.Millisecond + _secret))");
            var md5String = new string(md5Byte.SelectMany(s => $"{s:x2}").ToArray());
            var utf8 = HttpUtility.UrlEncode(source, Encoding.UTF8);

            var url =
                $"{API_URL}?q={utf8}&from={from}&to={to}&appid={_appId}&salt={_last.Millisecond}&sign={md5String}";
            ...
            var translateResult = JsonConvert.DeserializeObject<BaiduTranslateResult>(resultStr);
